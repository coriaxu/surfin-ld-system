...
**全新双模块分离架构（2026-01-05 重构）**：预算和开销现在是两个独立的管理模块，支持更精细的财务追踪。

- **📊 预算计划**：
  - 按月度设定预算上限
  - 区分"常规项目"与"特殊项目"
  - 自动从开销记录中计算已支出金额
  - 实时显示剩余预算，颜色标识（绿色有余/红色超支）
- **💳 开销记录**：
  - 逐笔记录每一笔实际支出
  - 记录开销日期、归属月份、金额、分类、用途说明
  - 支持编辑和删除单条开销
- **📈 预实对比**：
  - 可视化展示预算执行进度
  - 进度条颜色预警（绿色安全/黄色警告/红色超支）
  - 按月份+类型分组对比
...
### 6. 📖 PM 管理训练营案例库 (PM Case Library)

- **案例库门户**：展示 Surfin 管理训练营优秀学员的实战案例精选。
- **双分类展示**：
  - **🌍 跨文化管理案例**：肯尼亚高风险管理、跨文化任务辅导等。
  - **👔 管理者智慧案例**：从超级 PM 到指挥官、高效布置工作等。
- **国家管理手册**：印度尼西亚、肯尼亚、尼日利亚三国跨文化管理指南。
- **外链跳转**：一键进入完整案例库网站查看详情。
...
- 让对方**刷新页面**即可获取最新数据
- 现在系统对所有浏览器都采用**云端优先、缓存兜底**策略，能最大限度确保数据一致
...
### 2026-04-08：恢复管理者测评报告库门户入口

- 重新确认「管理者测评报告库」已做在本地 `index.html`，但线上 GitHub Pages 与 GitHub 主分支仍是旧版本，因此用户打开线上站点时看不到入口。
- 在「培训项目」下恢复「管理者测评报告库」导航入口，并保留与独立报告站 `surfin-assessment-hub` 的跳转关系。
- 同步补齐 2024 年入口状态，将 2024 从「待补录」改为「20 位管理者 / 报告已上线」，并添加 20 份 PDF 报告的直达入口。
- 2026-04-08 追加修正年度区：统计从 4 个测评年度改为 3 个已发生测评年度；2026 改为单独的下一轮提示，不再混入 2025/2024/2023 的年度卡片序列。
- 经验提醒：这类门户入口改动必须在本地验证后提交并推送到 GitHub，否则线上页面不会变化；独立报告站更新后，门户页也需要同步检查年份状态。

### 2026-01-17：金融小百科录入效率优化 ⚡️

- **自动化计算**：实现“查看率”与“下载率”的自动计算逻辑。
  - **公式**：查看率 = 已读人数 / 公司总人数；下载率 = 下载人数 / 公司总人数。
  - **实时响应**：监听已读人数、下载人数及总人数的变化，实现数据填入零延迟。
- **UI 交互改进**：
  - 计算字段设为 `readonly` 并辅以视觉置灰提示，防止误修改。
  - 移除了僵硬的固定总人数默认值，支持每期根据实际分母精准录入。
- **项目结构标准化**：启用 `AI_专属` 文件夹，统一管理任务计划、洞见及决策日志。

### 2026-01-05：预算与开销双模块分离 🎉

**重大架构升级**：将原有的"预算与开销"合并模块拆分为独立的双模块架构。

**问题背景**：

- 原有设计将预算和开销合并在一条记录中，无法单独追踪每一笔实际支出
- 用户反馈：产生实际开销时，无法记录明细（日期、用途等）

**解决方案（方案 B：双模块分离）**：

- **预算计划 Tab**：设定每月/每类预算上限，自动汇总实际支出
- **开销记录 Tab**：逐笔记录实际支出，包含日期、月份、分类、金额、用途
- **预实对比 Tab**：可视化进度条展示预算执行情况，颜色预警

**技术改动**：

- 新增 `expenses` 数组存储独立开销记录
- Dashboard 图表数据源改为从 expenses 汇总计算
- Excel 导出新增"开销明细"工作表
- 云同步逻辑兼容新数据结构

### 2025-12-30：月度数据结构化总结上线

- 新增 **月度总结** 按钮，支持一键生成当月数据报告
- **双格式导出**：
  - **PDF 洞察报告**：咨询公司风格深蓝主题，适合汇报分享
  - **Excel 数据源**：多 Sheet 结构（概览/培训明细/预算开销），适合数据分析
- **灵活配置**：可选择月份（最近 12 个月）、勾选包含模块
- **技术实现**：html2canvas 高保真渲染 + jsPDF + SheetJS

### 2025-12-30：PM 管理训练营案例库上线

- 新增 **PM 案例库** 模块，展示 Surfin 管理训练营优秀学员的实战案例
- **琥珀智库主题**：采用温暖的琥珀金配色，与金融小百科的翠绿色形成和谐差异
- **双分类展示**：跨文化管理案例 + 管理者智慧案例，Tab 切换浏览
- **国家管理手册**：印度尼西亚、肯尼亚、尼日利亚三国跨文化管理指南
- **一键跳转**：点击即可进入完整案例库网站 https://coriaxu.github.io/surfin-case-library/

> **📌 同步维护约定**：案例库网站更新后，学习发展系统的展示卡片需要手动同步。
> 徐老师只需说「案例库有更新了！」，若谷会自动访问案例库网站查看变化，然后同步更新到学习发展系统。
